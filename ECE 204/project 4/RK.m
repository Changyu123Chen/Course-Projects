function [err_a, err_b, integral_a, integral_b] = RK(t_max, n)
    a = [];
    b = [];
    a_true = [];
    b_true = [];
    ...
    for k = 1:n
        ...
        % a next
        s0_a=-1.456*y_a_curr;
        s0_b=(1.2345*cos(i*h)-1.123*(y_b_curr));
        s1_a=-1.456*(y_a_curr + 0.5*h*s0_a);
        s1_b=(1.2345*cos(i*h+0.5*h)-1.123*(y_b_curr+0.5*h*s0_b));
        s2_a=-1.456*(y_a_curr + 0.5*h*s1_a);
        s2_b=(1.2345*cos(i*h+0.5*h)-1.123*(y_b_curr+0.5*h*s1_b));
        s3_a=-1.456*(y_a_curr + h*s2_a);
        s3_b=(1.2345*cos(i*h+h)-1.123*(y_b_curr+h*s2_b));

        y_a_next = y_a_curr + h*(s0_a+2*s1_a+2*s2_a+s3_a)/6;
        a_diff = a_diff+(y_a_actual-y_a_curr)^2;
        y_a_curr = y_a_next;
        a = [a,y_a_next];
        % b next
        y_b_next = y_b_curr + h*(s0_b+2*s1_b+2*s2_b+s3_b)/6;
