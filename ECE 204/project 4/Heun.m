function [err_a, err_b, integral_a, integral_b] = Heun(t_max, n)
    a = [];
    b = [];
    a_true = [];
    b_true = [];
    ...
    for k = 1:n
        ...
        s1_a=-1.456*(y_a_curr + h*s0_a);
        s1_b=(1.2345*cos(i*h+h)-1.123*(y_b_curr+h*s0_b));
        y_a_next = y_a_curr + h*(s0_a+s1_a)/2;
        a_diff = a_diff+(y_a_actual-y_a_curr)^2;
        y_a_curr = y_a_next;
        a = [a,y_a_next];
        % b next
        y_b_next = y_b_curr + h*(s0_b+s1_b)/2;
